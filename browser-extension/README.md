...
它和主仓库的关系很简单：插件是入口层，主仓库是工作台层。插件负责在浏览器里识别页面、显示输入框旁的记忆建议、生成候选和诊断；主仓库负责本地 API、Viewer、审阅队列、记忆库、文档、打包和发布检查。插件不会脱离主仓库单独运行成一个云服务。
...
| AI 页面状态 | 检查 ChatGPT / Claude 等页面是否找到了输入框和最近对话；如果未命中，可复制诊断和保存证据命令用于修正规则 |
...
5. 打开同步侧栏查看“AI 页面状态”。如果显示“输入框：未找到”，点击“复制诊断”和“复制命令”，把诊断和证据保存结果交给开发者补站点规则。
