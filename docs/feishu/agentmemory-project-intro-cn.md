...
这张图补充解释本地工作台的日常使用路径：浏览器插件、本地会话导入和手动添加先把材料带进来；用户从总览进入待审阅队列，把网页片段和会话记录改成具体事实，再整理进记忆库、回看完整会话来源。可复用的方法沉淀为经验，未完成事项转成自然语言行动，成熟经验再生成 Skill 草稿。沉淀后的记忆、经验和行动会回到下一次对话，让 Agent 更快接上项目上下文。

| 工作台环节 | 用户看见什么 | 为什么重要 |
| --- | --- | --- |
| 浏览器插件 / 会话导入 | 网页内容、AI 对话、本地会话记录 | 先把真实上下文带进本地，而不是让用户手动复制整理 |
| 打开总览 | 最近会话、待处理入口、关键状态 | 每天从一个清晰入口开始，不从技术日志里找线索 |
| 处理待审阅 | 浏览器和会话候选、来源、时间 | 自动整理不等于自动乱存，长期记忆必须确认后保存 |
| 整理记忆库 | 身份、偏好、项目背景、来源和置信度 | 把网页片段改成具体事实，删掉临时链接和导航噪音 |
| 回看会话 | 完整时间线、浏览器对话、记录来源 | 需要知道一条记忆从哪段对话来，避免断章取义 |
| 沉淀经验 | 排布原则、交互原则、协作习惯 | 把一次对话总结成下次能复用的方法 |
| 转成行动 | 待跟进、正在推进、需要处理、已完成 | 用自然语言承接未完成事项，而不是暴露内部任务字段 |
| Skill 草稿 | 可复制的 `SKILL.md` 草稿 | 经验成熟后再人工写入本地 Skill，不自动污染本地目录 |
