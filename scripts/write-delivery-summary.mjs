...
- Confirm the external feedback template and GitHub issue template ask for \`turnCount\`, \`matchedSelectors.turn\`, and whether the candidate came from concrete conversation or selected text.
...
反馈时请尽量提供：浏览器版本、试用页面、问题步骤、同步侧栏诊断 JSON、截图或录屏。真实 AI 页面问题请保留 \`turnCount\` 和 \`matchedSelectors.turn\`，并说明候选记忆是否来自具体对话或用户选中文本。请不要提交私人聊天全文、Cookie、访问令牌、API Key、学校申请材料或任何敏感信息。
...
- 真实 AI 页面证据要求 \`turnCount > 0\`，候选记忆必须来自具体对话或用户选中文本，不能来自页面标题、链接或输入框草稿。
- 反馈模板和 GitHub Issue 模板会收集 \`turnCount\`、\`matchedSelectors.turn\` 和候选来源判断，方便维护者分诊会话抽取问题。
...
- 页面标题、链接、导航文案或输入框草稿不能作为公开发布通过证据里的记忆来源。
...
- 真实 AI 页面反馈需要保留 \`turnCount\` 和 \`matchedSelectors.turn\`，并说明候选是否来自具体对话或用户选中文本。
...
请先按 \`browser-extension/LOAD-THIS-FIRST.md\` 加载插件，再打开 Viewer 的本地 demo 完成一次“生成候选记忆 -> 修改草稿 -> 保存到待审阅队列”的闭环。真实 AI 页面测试请按 \`docs/browser-extension-ai-site-test-cards-cn.md\` 逐站记录；每个通过证据都必须来自具体对话或用户选中文本，并保留 \`turnCount > 0\` 的诊断字段。
`;
