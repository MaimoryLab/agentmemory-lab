  ...
  return (
    <main className="min-h-screen bg-[var(--app-bg)] text-neutral-950">
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        ...
        <nav className="sticky top-0 z-10 -mx-4 flex gap-1 overflow-x-auto border-b border-[var(--app-line)] bg-[var(--app-bg)]/95 px-4 py-3 backdrop-blur sm:mx-0 sm:px-0" aria-label={text.primaryNav}>
          <NavButton label={text.openView(text.todos)} active={view === "todos"} onClick={() => onView("todos")} icon={<CircleDot className="h-4 w-4" />}>{text.todos}</NavButton>
          <NavButton label={text.openView(text.sources)} active={view === "sources"} onClick={() => onView("sources")} icon={<FolderKanban className="h-4 w-4" />}>{text.sources}</NavButton>
          <NavButton label={text.openView(text.settings)} active={view === "settings"} onClick={() => onView("settings")} icon={<Settings className="h-4 w-4" />}>{text.settings}</NavButton>
        </nav>
