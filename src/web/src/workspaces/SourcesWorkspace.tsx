...
import { SourceIcon, sourceCount } from "../app/helpers.js";
import { Badge, Button, Card, Input, SectionTitle } from "../components/ui.js";
import { sourceLabel, type SourceFilter } from "../i18n/messages.js";
...
export function SourcesWorkspace({ sessions, sourceSummaries, sourceFilter, sessionOffset, observationsBySession, selectedSessionId, highlightedObservationId, onFilter, onLoadMore, onSelect }: {
    ...
    <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
      <Card className="min-w-0 p-3">
        <div className="mb-3 space-y-3 px-1">
          ...
          <label className="relative block">
            ...
            <Input aria-label={t.searchSources as string} placeholder={t.searchSources as string} value={query} onChange={(event) => setQuery(event.target.value)} className="pl-9" />
          </label>
          <div className="flex gap-1 overflow-x-auto" aria-label={t.sourceFilter as string}>
            {filters.map((filter) => (
              <button
                key={filter}
                type="button"
              ...
              >
                {filter === "all" ? t.all : sourceLabel(filter, locale)}
        ...
        <div className="max-h-[calc(100vh-220px)] space-y-2 overflow-y-auto pr-1">
          {sessions.length === 0 && <div className="rounded-md bg-neutral-50 p-4 text-sm text-neutral-600">{t.connectSource}</div>}
          {sessions.length > 0 && groups.length === 0 && <div className="rounded-md bg-neutral-50 p-4 text-sm text-neutral-600">{t.noSessionMatches}</div>}
          {groups.map((group) => {
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block text-xs text-neutral-500">{group.sessions.length} {t.sessions}</span>
                    </span>
                  </span>
                ...
                <div className="divide-y divide-neutral-100">
                  {visibleSessions.map((session) => (
                    ...
                    >
                      ...
                      <div className="mt-1 truncate text-sm text-neutral-600">{session.preview || t.temporarySession}</div>
                      <div className="mt-2 text-xs text-neutral-400">{session.observationCount} {t.messages}</div>
                    </button>
                  ))}
                  {hiddenCount > 0 && (
                    <button
                    ...
                    >
                      {(t.showMoreSessions as (count: number) => string)(hiddenCount)}
                    </button>
                  )}
                </div>
              </section>
      ...
      <Card className="min-w-0 p-4">
        {selected ? (
          <>
            <div className="mb-4 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <h2 className="truncate text-xl font-semibold tracking-normal">{selected.preview || t.temporarySession}</h2>
              </div>
              <Badge>{selected.observationCount} {t.messages}</Badge>
            </div>
            <div className="max-h-[calc(100vh-220px)] space-y-3 overflow-y-auto pr-1">
              {observations.length === 0 && <div className="rounded-md bg-neutral-50 p-4 text-sm text-neutral-600">{t.selectSource}</div>}
              {visibleObservations.map((observation) => (
                ...
                >
                  <div className="mb-2 flex items-center justify-between gap-2 text-xs text-neutral-500">
                    <span className="capitalize">{observation.role === "unknown" ? t.message : observation.role}</span>
                    <time>{new Date(observation.createdAt).toLocaleString(locale)}</time>
                  </div>
          ...
          <div className="rounded-md bg-neutral-50 p-4 text-sm text-neutral-600">{t.noSourceSessions}</div>
        )}
      </Card>
    </div>
  );
...
function sessionGroups(sessions: SessionRecord[], locale: "zh-CN" | "en-US"): Array<{ key: string; label: string; sessions: SessionRecord[] }> {
...
function matchesSessionQuery(session: SessionRecord, query: string, locale: "zh-CN" | "en-US"): boolean {
...
function sessionProjectLabel(session: SessionRecord, locale: "zh-CN" | "en-US"): string {
  if (session.source === "browser") return session.path === "browser" ? sourceLabel("browser", locale) : readablePathSegment(session.path, locale);
...
function readablePathSegment(value: string | undefined, locale: "zh-CN" | "en-US"): string {
  if (!value) return locale === "zh-CN" ? "临时会话" : "Temporary session";
  return value.replace(/\.jsonl$/u, "").replace(/^-+|-+$/gu, "").replace(/[-_]+/gu, " ") || (locale === "zh-CN" ? "临时会话" : "Temporary session");
}
