...
describe("viewer request handler DNS rebinding defence (e2e)", () => {
  ...
  it("serves actions and frontier from viewer KV fallback when REST proxy misses", async () => {
    ...
    expect(actionsBody.actions[0].title).toBe("整理验收截图");
    expect(actionsBody.todoExtract).toMatchObject({ status: "idle" });
  ...
  it("expires stale todo extraction running status from viewer KV fallback", async () => {
    ...
    try {
      ...
      expect(actions.status).toBe(200);
      expect(actionsBody.todoExtract).toMatchObject({
        status: "error",
        startedAt,
  ...
  it("generates todos through the viewer fallback endpoint when REST proxy misses", async () => {
    ...
    try {
      ...
      expect(actionsBody.todoExtract).toMatchObject({ status: "done" });
